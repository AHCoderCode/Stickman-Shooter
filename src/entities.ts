...
export class PlayerController extends Phaser.Physics.Arcade.Sprite {
  ...
  private isLedgeGrabbing = false;
  private isClimbingLedge = false;
  private isTouchingWall = false;
  private ledgeGrabPoint = new Phaser.Math.Vector2();
  ...
  constructor(
    ...
    body.setSize(24, 40);
    body.setOffset(4, 8);
    body.setMaxVelocityX(config.maxSpeed);
    ...
    scene.input.on('wheel', (pointer: any, gameObjects: any, deltaX: number, deltaY: number) =>
      this.weaponManager.cycleWeapon(deltaY > 0 ? 1 : -1)
    );
    scene.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
      if (!this.isPointerOverUI(pointer)) this.weaponManager.startFiring();
    });
  ...
  private isPointerOverUI(pointer: Phaser.Input.Pointer): boolean {
    ...
    for (const t of targets) {
      if (t.gameObject && (t.gameObject.type === 'Container' || t.gameObject.input?.draggable === false))
        return true;
    }
    return false;
  }
  ...
  getBarrelPosition() {
    return new Phaser.Math.Vector2(this.x + (this.facingRight ? 20 : -20), this.y - 5);
  }
  takeDamage(amount: number) {
    this.health.takeDamage({ amount, source: this, damageType: DamageType.Bullet });
  }
  ...
  private updateTimers(dt: number) {
    ...
    if (this.isSliding) {
      this.slideTimer -= dt;
      if (this.slideTimer <= 0 || !this.keys.down.isDown) {
        this.isSliding = false;
        this.setCrouchCollider(false);
      }
    }
    if (this.isClimbingLedge) {
      this.ledgeClimbTimer -= dt;
      if (this.ledgeClimbTimer <= 0) {
        this.isClimbingLedge = false;
        this.isLedgeGrabbing = false;
      }
    }
  }

  private updateWallDetection() {
    const body = this.body as Phaser.Physics.Arcade.Body;
  ...
  private updateState(grounded: boolean) {
    ...
    if (!this.isDashing && !this.isClimbingLedge && !this.isLedgeGrabbing) {
      ...
      else if (this.isTouchingWall && !grounded && Math.abs(moveInput) > 0.1)
      ...
      else if (grounded && this.keys.down.isDown && Math.abs(moveInput) <= 0.1) {
        this.currentState = PlayerState.Crouch;
        this.setCrouchCollider(true);
  ...
  private updateFacing() {
    if (this.isDashing || this.currentState === PlayerState.WallSlide) return;
    const mi = this.getMoveInput();
    if (mi > 0.1 && !this.facingRight) this.setFacing(true);
    else if (mi < -0.1 && this.facingRight) this.setFacing(false);
  }
  ...
  private getMoveInput() {
    return (this.keys.left.isDown ? -1 : 0) + (this.keys.right.isDown ? 1 : 0);
  }

  private applyMovement(dt: number, grounded: boolean) {
    const body = this.body as Phaser.Physics.Arcade.Body;
    if (this.isDashing || this.isClimbingLedge || this.isLedgeGrabbing) return;
    if (this.currentState === PlayerState.WallSlide) {
      body.velocity.y = Math.min(body.velocity.y, this.config.wallSlideSpeed);
      return;
    }
    if (this.currentState === PlayerState.Slide) {
      body.velocity.x = this.facingRight ? this.config.slideSpeed : -this.config.slideSpeed;
      return;
    }
    const mi = this.getMoveInput();
    const target = mi * this.config.maxSpeed;
  ...
  private applyJump(grounded: boolean) {
    ...
    if (this.jumpBufferTimer > 0 && this.coyoteTimer > 0 && !this.isDashing) {
      this.executeJump(this.config.jumpForce);
      this.jumpBufferTimer = 0;
      this.coyoteTimer = 0;
      this.hasDoubleJumped = false;
      return;
    }
    if (this.jumpBufferTimer > 0 && this.isTouchingWall && !grounded && this.wallJumpLockTimer <= 0) {
      const dir = this.facingRight ? -1 : 1;
      body.velocity.x = dir * this.config.wallJumpForceX;
      body.velocity.y = this.config.wallJumpForceY;
    ...
    if (
      ...
      this.config.doubleJumpUnlocked &&
      this.wallJumpLockTimer <= 0
    ) {
      this.executeJump(this.config.doubleJumpForce);
  ...
  private executeJump(force: number) {
    (this.body as Phaser.Physics.Arcade.Body).velocity.y = force;
  }
  ...
  private applyDash() {
    ...
    const body = this.body as Phaser.Physics.Arcade.Body;
    body.velocity.x = this.facingRight ? this.config.dashSpeed : -this.config.dashSpeed;
    body.velocity.y = 0;
  }
  private applyWallSlide(grounded: boolean) {}
  private applySlide(grounded: boolean) {}
  private setCrouchCollider(crouching: boolean) {
    const body = this.body as Phaser.Physics.Arcade.Body;
    if (crouching) {
      body.setSize(24, 24);
      body.setOffset(4, 24);
  ...
  private updateLedgeGrab(grounded: boolean) {
    ...
    const dir = this.facingRight ? 1 : -1;
    const originX = this.x + dir * 10;
    const originY = this.y - 20;
    const wallHit = this.scene.physics.raycast(originX, originY, originX + dir * 30, originY, 5);
    if (!wallHit) return;
    const ledgeHit = this.scene.physics.raycast(
      wallHit.point.x,
      wallHit.point.y - 5,
      wallHit.point.x,
      wallHit.point.y - 40,
    ...
    this.ledgeGrabPoint.set(wallHit.point.x, wallHit.point.y);
    (this.body as Phaser.Physics.Arcade.Body).velocity.set(0, 0);
    (this.body as Phaser.Physics.Arcade.Body).allowGravity = false;
    this.currentState = PlayerState.LedgeGrab;
    if (Phaser.Input.Keyboard.JustDown(this.keys.jump)) {
      this.isClimbingLedge = true;
      this.ledgeClimbTimer = this.config.ledgeClimbDuration;
      this.isLedgeGrabbing = false;
      (this.body as Phaser.Physics.Arcade.Body).allowGravity = true;
      this.currentState = PlayerState.LedgeClimb;
    }
  }
  private applyLedgeClimb(dt: number) {
    ...
    this.setPosition(
      Phaser.Math.Linear(this.x, target.x, dt * 10),
      Phaser.Math.Linear(this.y, target.y, dt * 10)
    );
  ...
  private handleDeath = (payload: { target: any; source: unknown }) => {
    if (payload.target === this.health) {
      this.currentState = PlayerState.Dead;
      this.setTint(0xff0000);
      (this.body as Phaser.Physics.Arcade.Body).setAngularVelocity(360);
      this.scene.tweens.add({ targets: this, alpha: 0, duration: 1000, onComplete: () => this.destroy() });
    }
  };
...
export class AIStickman extends Phaser.Physics.Arcade.Sprite {
  ...
  constructor(
    ...
    body.setSize(24, 40);
    body.setOffset(4, 8);
    body.setMaxVelocityX(archetype.moveSpeed * 1.5);
    body.setMaxVelocityY(1200);
  ...
  getWeaponConfig() {
    const w = this.weaponManager.getCurrentWeapon();
    return w ? (w as any).config : weaponConfigs[0];
  }
  fireWeapon() {
    this.weaponManager.getCurrentWeapon()?.fire();
  }
  jump() {
    const body = this.body as Phaser.Physics.Arcade.Body;
    if (body.blocked.down) body.velocity.y = -500;
  }
...
export class Projectile extends Phaser.Physics.Arcade.Sprite {
  ...
  static spawn(
    ...
    if (Projectile.pool.length === 0) {
      for (let i = 0; i < Projectile.POOL_SIZE; i++) {
        const p = new Projectile(scene, 0, 0, texture);
  ...
  preUpdate(time: number, delta: number) {
    ...
    if (this.lifeTimer <= 0) {
      Projectile.release(this);
    } else {
      const rect = new Phaser.Geom.Rectangle(this.x - 4, this.y - 4, 8, 8);
      const bodies = this.scene.physics.overlapRect(rect.x, rect.y, rect.width, rect.height, true, true);
...
export class Destructible extends Phaser.Physics.Arcade.Sprite {
  ...
  private handleDeath = (payload: { target: any }) => {
    if (payload.target === this.health) {
      this.destroy();
    }
  };
